...
-- AVERAGE MONEY SPENT
SELECT DC.C_ID, DC.C_NAME, DC.C_EMAIL, AVG(FS.Total_Sales) AS AVG_Spent
FROM FactSales FS
JOIN DIM_CUSTOMER DC ON FS.C_ID = DC.C_ID
GROUP BY DC.C_ID, DC.C_NAME, DC.C_EMAIL
ORDER BY AVG_Spent DESC;
-- DAYS CUSTOMER VISITED
SELECT DC.C_ID, DC.C_NAME, DC.C_EMAIL, COUNT(FS.SERVICE_CHARGE) AS DAYS_VISITED
FROM FactSales FS
JOIN DIM_CUSTOMER DC ON FS.C_ID = DC.C_ID
GROUP BY DC.C_ID, DC.C_NAME, DC.C_EMAIL
ORDER BY DAYS_VISITED DESC;

-- Query to Analyze Purchases Over Time
SELECT DISTINCT DC.C_NAME, DV.MAKE AS Vehicle, DS.S_TYPE AS Service_Purchased, DP.P_NAME AS Part_Purchased, 
DD.DATE AS Date_Of_Purchase, FS.Service_Charge, FS.Parts_Charge, FS.Total_Sales
FROM FactSales FS
JOIN DIM_CUSTOMER DC ON FS.C_ID = DC.C_ID
JOIN DIM_VEHICLE DV ON FS.V_ID = DV.V_ID
JOIN DIM_SERVICE DS ON FS.S_ID = DS.S_ID
JOIN DIM_PART DP ON FS.P_ID = DP.P_ID
JOIN DIM_DATE DD ON FS.D_ID = DD.D_ID
ORDER BY DC.C_NAME, DD.DATE;

SHOW TABLES;

SELECT * FROM DIM_VEHICLE;
-- COUNT OF MAKES
SELECT MAKE, COUNT(V_ID) AS NUMBER_OF_MAKES
FROM DIM_VEHICLE
GROUP BY MAKE
ORDER BY NUMBER_OF_MAKES DESC;

-- NUMBER OF MAKES AND MODELS
SELECT MAKE, MODEL, COUNT(V_ID) AS Number_Of_Vehicles
FROM DIM_VEHICLE
GROUP BY MAKE, MODEL
ORDER BY Number_Of_Vehicles DESC;

-- Most Frequently Performed Services
SELECT DS.S_TYPE AS Service_Type, COUNT(FS.S_ID) AS Number_Of_Times_Performed
FROM FactSales FS
JOIN DIM_SERVICE DS ON FS.S_ID = DS.S_ID
GROUP BY DS.S_TYPE
ORDER BY Number_Of_Times_Performed DESC;

-- Revenue Generated by Each Service
SELECT DS.S_TYPE AS Service_Type, SUM(FS.Service_Charge) AS Total_Revenue
FROM FactSales FS
JOIN DIM_SERVICE DS ON FS.S_ID = DS.S_ID
GROUP BY DS.S_TYPE
...
SELECT DS.S_TYPE AS Service_Type, AVG(FS.Service_Charge) AS Average_Revenue_Per_Service
FROM FactSales FS
JOIN DIM_SERVICE DS ON FS.S_ID = DS.S_ID
GROUP BY DS.S_TYPE
ORDER BY Average_Revenue_Per_Service DESC;

-- Number of Services Per Customer
SELECT DISTINCT DC.C_NAME AS Customer_Name, DS.S_TYPE AS Service_Type, COUNT(FS.S_ID) AS Number_Of_Services_Performed
FROM FactSales FS
JOIN DIM_CUSTOMER DC ON FS.C_ID = DC.C_ID
JOIN DIM_SERVICE DS ON FS.S_ID = DS.S_ID
GROUP BY DC.C_NAME, DS.S_TYPE
ORDER BY Number_Of_Services_Performed DESC;

-- TOP PARTS USED
SELECT DISTINCT DC.C_NAME AS Customer_Name, DP.P_NAME AS PART_NAME, COUNT(FS.P_ID) AS Number_Of_PARTS_USED
FROM FactSales FS
JOIN DIM_CUSTOMER DC ON FS.C_ID = DC.C_ID
JOIN DIM_PART DP ON FS.P_ID = DP.P_ID
GROUP BY DC.C_NAME, DP.P_NAME
ORDER BY Number_Of_PARTS_USED DESC;

-- REVENUE FOR PARTS
SELECT DP.P_NAME AS PART_NAME, SUM(FS.Service_Charge) AS Total_Revenue
FROM FactSales FS
JOIN DIM_PART DP ON FS.P_ID = DP.P_ID
GROUP BY DP.P_NAME
ORDER BY Total_Revenue DESC;
